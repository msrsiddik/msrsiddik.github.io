...
(function () {
  ...
  // ===========================================================================
  // Select-to-ask: selecting text anywhere in the page content (not the chat
  // panel itself, to avoid a selection-on-selection loop) shows a floating
  // "Ask AI about this" button. Clicking it drops the selected text into the
  // chat input — prefilled, not auto-sent — so the visitor can add their own
  // question around it before submitting.
  // ===========================================================================
  (function initSelectToAsk() {
    if (!chatInput) return;
    const contentRoot = document.querySelector('[data-page-content]');
    if (!contentRoot) return;

    // Sparkle glyph marks this as an AI action, matching the "generated by
    // AI" framing used elsewhere in the chat panel.
    const sparkleIcon = '<svg class="select-ask-icon" width="14" height="14" viewBox="0 0 16 16" fill="currentColor" aria-hidden="true">' +
      '<path d="M8 0l1.2 4.8L14 6l-4.8 1.2L8 12l-1.2-4.8L2 6l4.8-1.2L8 0z"/>' +
      '<path d="M13 9l.6 2.4L16 12l-2.4.6L13 15l-.6-2.4L10 12l2.4-.6L13 9z" opacity=".7"/>' +
      '</svg>';
    const btnLabel = () => sparkleIcon + '<span>' + (currentLabels.selectAsk || 'Ask AI') + '</span>';

    const btn = document.createElement('button');
    btn.type = 'button';
    btn.className = 'select-ask-btn';
    ...
    function hideBtn() { btn.hidden = true; pendingText = ''; }

    function showBtnForSelection() {
      const sel = window.getSelection();
      const text = sel && sel.toString().trim();
      if (!text || !contentRoot.contains(sel.anchorNode)) { hideBtn(); return; }
    ...
    document.addEventListener('mouseup', (e) => {
      if (e.target === btn) return; // let the click handler fire first
      setTimeout(showBtnForSelection, 0); // let selection settle after the click
    });
    document.addEventListener('keyup', (e) => {
      if (e.shiftKey || e.key === 'Shift') setTimeout(showBtnForSelection, 0);
    });
